---
title: "Case Study B - Mice"
author: "Emily Schmidt"
date: "`r Sys.Date()`"
output: html_document
---

*Case Study B* - Pain Relief Medication\

• In the drug development process one of the important steps is to determine the
minimum dosage amount of the drug that achieves efficacy. Sometimes this may
involve estimating combinations of drugs that achieve maximum efficacy with a
minimum drug amount.\

• The experiment in this case study consists of administering a combination of two
drugs to 10 mice at different doses. The drugs are morphine and marijuana and are
used as pain relief medication (‘tranquilizers’). In order to measure their effect ‘flick tail’ tests were performed for two drugs and their combination.\

• The client’s objective is to detect whether a synergy exists between these two drugs.
In addition, the client is also interested to know the effective dosage at which 50%
of the subjects would be expected to respond.\

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
knitr::opts_knit$set(root.dir = "C:/Users/Manu/Desktop/master course/2nd semester/analytics consulting")
```
...
#setwd("C:\\Users\\emann\\R Data Files") # Set directory
#getwd() # Check the working directory
```

```{r}
# Load your data, Bpain.csv
pain = read.table("Bpain.csv", sep = ",", header = TRUE)
pain1 = read.table("Bpain.csv", sep = ",", header = TRUE)
```

```{r}
# Moved reps to the end of the dataframe
pain <- pain[, c(1:5, ncol(pain), 6:(ncol(pain)-1))]
...
```{r, message=FALSE, warning=FALSE}
pain_reps = pain[-5]
cormat <- round(cor(pain_reps),2) # Round the correlation coefficient to two decimal places
...
tbf here linear programming could be very helpful but I wan to wait to speak with you 
